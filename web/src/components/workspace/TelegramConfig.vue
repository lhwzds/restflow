    ...
    if (botToken.value) {
      if (hasExistingToken.value) {
        await updateSecret(
          ...
          botToken.value,
          'Telegram bot token for task notifications',
        )
      } else {
        await createSecret(
          TELEGRAM_BOT_TOKEN_KEY,
    ...
    if (chatId.value) {
      if (hasExistingChatId.value) {
        await updateSecret(
          ...
          chatId.value,
          'Default Telegram chat ID for task notifications',
        )
      } else {
        await createSecret(
          TELEGRAM_CHAT_ID_KEY,
...
<template>
  <div class="space-y-4">
    ...
    <p class="text-xs text-muted-foreground">
      Configure Telegram to receive task notifications. Get your bot token from
