...
│   │   │   ├── models/            #   Data models (30+ model files)
...
Level 0 (Foundation):  restflow-traits    restflow-sandbox
                            │                   │
Level 1 (Utility):    restflow-storage ─────────┘
                       │         │
Level 2 (Framework):   │    restflow-ai
                       │    │    restflow-tools
                       │    │         │
...
**Note**: `restflow-tools` only depends on `restflow-ai` in `[dev-dependencies]` (for test mocks like `MockLlmClient`). There is no production dependency from tools to ai.
...
### Type-safe workflow:
1. Define/modify Rust types in `crates/restflow-core/src/models/`
2. Run type generation
3. TypeScript compiler ensures frontend compatibility
