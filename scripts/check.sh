...
PYTHON_VENV="${PYTHON_VENV:-/tmp/restflow-python-venv}"
CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/restflow-python-target}"
WHEEL_DIR="${WHEEL_DIR:-/tmp/restflow-python-dist}"
...
rust_native_checks() {
  cargo test --manifest-path "${PROJECT_ROOT}/Cargo.toml" -p restflow-native --features python-module
  cargo clippy --manifest-path "${PROJECT_ROOT}/Cargo.toml" -p restflow-native --features python-module --all-targets -- -D warnings
}
...
Environment:
  ...
  PYTHON_VENV      Virtualenv path. Default: /tmp/restflow-python-venv
  CARGO_TARGET_DIR Cargo target path. Default: /tmp/restflow-python-target
  WHEEL_DIR        Wheel output path. Default: /tmp/restflow-python-dist
EOF
}
