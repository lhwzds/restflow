//! # codocia
//!
//! CLI owns the minimal V2 executable skill command loop.
//!
//! ## Owns
...
fn print_usage() {
    println!(
        r#"Usage:
  restflow-v2 skill new --kind rust_binary|python_uv --id <id> [--name <name>] [--version <version>] <dir>
  restflow-v2 skill build [--profile release] [--target-dir <dir>] <dir>
  restflow-v2 skill run [--input <json-object>] [--timeout <seconds>] <dir>
  restflow-v2 skill install-local [--root <dir>] [--id <id>] [--overwrite] <dir>
  restflow-v2 skill list [--root <dir>]
"#
    );
}
