...
/// Get API key from RestFlow auth profile (local storage)
async fn get_api_key_from_profile_local(
    core: &Arc<AppCore>,
...
/// Get API key from RestFlow auth profile (daemon IPC)
async fn get_api_key_from_profile_remote(
    ...
    client.get_api_key_for_profile(profile.id).await
}
...
async fn resolve_session_id_local(
    core: &Arc<AppCore>,
    args: &ClaudeArgs,
) -> Result<Option<String>> {
...
async fn resolve_session_id_remote(
    client: &mut IpcClient,
    args: &ClaudeArgs,
) -> Result<Option<String>> {
    if args.new_session && args.session.is_some() {
...
fn save_conversation_local(
    ...
    session.add_message(restflow_core::models::chat_session::ChatMessage::user(user_input));
    session.add_message(restflow_core::models::chat_session::ChatMessage::assistant(
        assistant_output,
    ));
...
async fn save_conversation_remote(
    client: &mut IpcClient,
    session_id: &str,
    user_input: &str,
    assistant_output: &str,
...
pub async fn run(core: Arc<AppCore>, args: ClaudeArgs, format: OutputFormat) -> Result<()> {
    ...
    let session_id = match ipc_client.as_mut() {
        Some(client) => resolve_session_id_remote(client, &args).await?,
    ...
    let oauth_token = match ipc_client.as_mut() {
        Some(client) => {
            get_api_key_from_profile_remote(client, args.auth_profile.as_deref()).await?
        }
        None => get_api_key_from_profile_local(&core, args.auth_profile.as_deref()).await?,
    };
    ...
    if let Some(ref id) = session_id
        && let Some(text) = result.get_text()
    {
        let save_result = match ipc_client.as_mut() {
            Some(client) => save_conversation_remote(client, id, &prompt, text).await,
            None => save_conversation_local(&core, id, &prompt, text),
        };
