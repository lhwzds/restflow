...
use restflow_core::auth::{
    ...
    ProfileUpdate, SecureCredential,
};
...
pub async fn run(core: Arc<AppCore>, command: KeyCommands, format: OutputFormat) -> Result<()> {
    ...
    match command {
        KeyCommands::Add {
            ...
            key,
            name,
        } => add_key(&manager, &provider, &key, name, format).await,
...
async fn show_key(manager: &AuthProfileManager, id: &str, format: OutputFormat) -> Result<()> {
...
async fn use_key(manager: &AuthProfileManager, id: &str, format: OutputFormat) -> Result<()> {
    let resolved_id = resolve_profile_id(manager, id).await?;
    let profile = manager
        .get_profile(&resolved_id)
        .await
...
async fn remove_key(manager: &AuthProfileManager, id: &str, format: OutputFormat) -> Result<()> {
    let resolved_id = resolve_profile_id(manager, id).await?;
...
async fn test_key(manager: &AuthProfileManager, id: &str, format: OutputFormat) -> Result<()> {
    let resolved_id = resolve_profile_id(manager, id).await?;
    let profile = manager
        .get_profile(&resolved_id)
        .await
