...
pub(crate) const VALID_TOP_LEVEL_FIELDS: &str =
    "system.*, agent.*, api.*, runtime.*, channel.*, registry.*, cli.*";
...
pub(crate) const VALID_CLI_FIELDS: &str = "cli.version, cli.agent, cli.model, cli.sandbox.enabled, cli.sandbox.env.isolate, cli.sandbox.env.allow, cli.sandbox.env.block, cli.sandbox.limits.timeout_secs, cli.sandbox.limits.max_output_bytes";
