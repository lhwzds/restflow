...
use runtime::daemon::{IpcClient, is_daemon_available};
use runtime::paths;
use std::path::Path;
...
fn format_health(health: &runtime::auth::ProfileHealth) -> String {
    format!("{health:?}")
}
...
fn summary_from_profiles(profiles: &[runtime::auth::AuthProfile]) -> ManagerSummary {
