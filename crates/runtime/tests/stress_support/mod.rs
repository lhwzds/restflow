...
use ai::tools::{Tool, ToolOutput, ToolRegistry, ToolResult};
use anyhow::{Result, anyhow};
use async_trait::async_trait;
use futures::stream;
use runtime::auth::AuthProfileManager;
use runtime::models::{AgentNode, TaskSchedule, TaskStatus};
use runtime::process::ProcessRegistry;
use runtime::prompt_files;
use runtime::runtime::agent::install_test_tool_overrides;
use runtime::runtime::subagent::AgentDefinitionRegistry;
use runtime::runtime::task_runtime::install_test_llm_factory;
use runtime::runtime::task_runtime::runner::{AgentExecutor, ExecutionResult};
use runtime::runtime::task_runtime::testkit::{MockNotificationSender, create_test_storage};
use runtime::runtime::{AgentRuntimeExecutor, TaskRunner, TaskRunnerConfig};
...
impl LlmClient for ScriptedLlmClient {
    ...
    async fn complete(&self, request: CompletionRequest) -> ai::Result<CompletionResponse> {
        ...
        let content = request
            ...
            .iter()
            .rev()
            .find(|message| matches!(message.role, ai::Role::User))
...
impl AgentExecutor for ProviderAwareMockExecutor {
    async fn execute(
        ...
        task_id: Option<&str>,
        input: Option<&str>,
        _memory_config: &runtime::models::MemoryConfig,
        _steer_rx: Option<mpsc::Receiver<runtime::models::SteerMessage>>,
...
pub fn install_real_io_tool_overrides() -> (
    runtime::runtime::agent::TestToolOverrideGuard,
...
pub async fn run_chat_workload_with_tools(
    ...
    for round in 0..rounds {
        ...
        let executor = ai::AgentExecutor::new(llm, tools.clone());
        let result = executor
            .run(ai::AgentConfig::new(format!("stress chat round {round}")))
            .await
...
pub async fn run_task_workload_with_real_runtime(
    ...
    let model = runtime::ModelId::from_api_name(profile.model_id)
        .or_else(|| runtime::ModelId::from_serialized_str(profile.model_id))
        .expect("known model");
