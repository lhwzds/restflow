...
//! The shared event domain lives in `ai::telemetry`.
...
mod tests {
    ...
    use ai::telemetry::TelemetrySink;
    ...
    async fn sink_projects_trace_session_metrics_and_health() {
        ...
        let trace = ai::telemetry::RestflowTrace::new("run-1", "session-1", "session-1", "agent-1");
        sink.emit(
            ai::telemetry::ExecutionEventEnvelope::new(
                trace,
        ...
        let llm_trace =
            ai::telemetry::RestflowTrace::new("run-1", "session-1", "session-1", "agent-1");
        sink.emit(
            ai::telemetry::ExecutionEventEnvelope::new(
                llm_trace,
                ai::telemetry::ExecutionEvent::LlmCall(ai::telemetry::LlmCallPayload {
