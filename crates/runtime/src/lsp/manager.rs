...
impl DiagnosticsProvider for LspManager {
    async fn ensure_open(&self, path: &Path) -> tools::Result<()> {
        self.ensure_open(path)
            .await
            .map_err(|e| tools::ToolError::Tool(e.to_string()))
    }

    async fn did_change(&self, path: &Path, content: &str) -> tools::Result<()> {
    ...
    async fn wait_for_diagnostics(
        ...
        path: &Path,
        timeout: Duration,
    ) -> tools::Result<Vec<Diagnostic>> {
        self.wait_for_diagnostics(path, timeout)
            .await
            .map_err(|e| tools::ToolError::Tool(e.to_string()))
    }

    async fn get_diagnostics(&self, path: &Path) -> tools::Result<Vec<Diagnostic>> {
