...
use crate::tools::{Result, Tool, ToolOutput};
...
impl Tool for MultiEditTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolOutput> {
        let file_path = args
            .get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::tools::ToolError::Tool("Missing 'file_path' argument".into()))?;

        let edits = args
            .get("edits")
            .and_then(|v| v.as_array())
            .ok_or_else(|| {
                crate::tools::ToolError::Tool("Missing 'edits' array argument".into())
            })?;
        ...
        for (i, edit) in edits.iter().enumerate() {
            let old_string = edit
                ...
                .and_then(|v| v.as_str())
                .ok_or_else(|| {
                    crate::tools::ToolError::Tool(format!("Edit {i}: missing 'old_string'"))
                })?;
            let new_string = edit
                .get("new_string")
                .and_then(|v| v.as_str())
                .ok_or_else(|| {
                    crate::tools::ToolError::Tool(format!("Edit {i}: missing 'new_string'"))
                })?;
