...
use types::{LlmProvider, catalog, provider_meta};
...
fn test_build_model_specs_contains_codex_cli() {
    ...
    assert!(specs.iter().any(|spec| spec.name == "gpt-5-4"
        ...
        && spec.client_kind == types::ClientKind::Http));
    assert!(specs.iter().any(|spec| spec.name == "gpt-5-4-mini"
        && spec.client_model == "gpt-5.4-mini"
        && spec.client_kind == types::ClientKind::Http));
    assert!(specs.iter().any(|spec| spec.name == "gpt-5.4"
        && spec.client_model == "gpt-5.4"
        && spec.client_kind == types::ClientKind::CodexCli));
    assert!(specs.iter().any(|spec| spec.name == "gpt-5.4-mini"
        && spec.client_model == "gpt-5.4-mini"
        && spec.client_kind == types::ClientKind::CodexCli));
    assert!(
        specs
            .iter()
            .any(|spec| spec.name == "gpt-5-codex"
                && spec.client_kind == types::ClientKind::CodexCli)
    );
    assert!(specs.iter().any(
        |spec| spec.name == "gpt-5.1-codex" && spec.client_kind == types::ClientKind::CodexCli
    ));
    assert!(specs.iter().any(
        |spec| spec.name == "gpt-5.2-codex" && spec.client_kind == types::ClientKind::CodexCli
    ));
    assert!(specs.iter().any(
        |spec| spec.name == "gpt-5.3-codex" && spec.client_kind == types::ClientKind::CodexCli
    ));
}
