...
use std::sync::Arc;
use types::store::SecurityQueryProvider;
...
impl SecurityQueryProvider for SecurityQueryProviderAdapter {
    fn show_policy(&self) -> tools::Result<Value> {
        let policy = crate::models::SecurityPolicy::default();
        Ok(serde_json::to_value(policy)?)
    }

    fn list_permissions(&self) -> tools::Result<Value> {
        let policy = crate::models::SecurityPolicy::default();
        Ok(json!({
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = tools::Result<Value>> + Send + '_>>
    {
        ...
        Box::pin(async move {
            ...
            let ai_action = types::security::ToolAction {
