...
use types::store::{AuthProfileCreateRequest, AuthProfileStore, AuthProfileTestRequest};
...
impl AuthProfileStore for AuthProfileStorageAdapter {
    fn list_profiles(&self) -> tools::Result<Value> {
    ...
    fn add_profile(&self, request: AuthProfileCreateRequest) -> tools::Result<Value> {
        let key_name = format!(
            "{}_API_KEY",
            request.provider.to_uppercase().replace('-', "_")
        );
        let secret_value = match &request.credential {
            types::store::CredentialInput::ApiKey { key, .. } => key.clone(),
            types::store::CredentialInput::Token { token, .. } => token.clone(),
            types::store::CredentialInput::OAuth { access_token, .. } => access_token.clone(),
        };
    ...
    fn remove_profile(&self, id: &str) -> tools::Result<Value> {
        self.storage.delete_secret(id)?;
        Ok(json!({ "id": id, "removed": true }))
    }

    fn test_profile(&self, request: AuthProfileTestRequest) -> tools::Result<Value> {
...
mod tests {
    ...
    use types::store::{AuthProfileStore, CredentialInput};
