...
impl TerminalStore for TerminalStoreAdapter {
    fn create_session(
        ...
        let default_name = self.storage.get_next_name()?;
        let mut session = TerminalSession::new(id, name.unwrap_or(&default_name).to_string());
        session.set_config(
    ...
    fn list_sessions(&self) -> tools::Result<Value> {
        let sessions = self.storage.list()?;
        Ok(serde_json::to_value(sessions)?)
    }

    fn send_input(&self, session_id: &str, data: &str) -> tools::Result<Value> {
        let mut session = self.storage.get(session_id)?.ok_or_else(|| {
    ...
    fn read_output(&self, session_id: &str) -> tools::Result<Value> {
        let session = self.storage.get(session_id)?.ok_or_else(|| {
            ToolError::Tool(format!("Terminal session not found: {}", session_id))
        })?;
        Ok(json!({
            "session_id": session_id,
    ...
    fn close_session(&self, session_id: &str) -> tools::Result<Value> {
        let mut session = self.storage.get(session_id)?.ok_or_else(|| {
            ToolError::Tool(format!("Terminal session not found: {}", session_id))
        })?;
...
mod tests {
    ...
    fn test_close_session() {
        let (adapter, _dir) = setup();
        let created = adapter
            .create_session(Some("closeable"), None, None)
            .unwrap();
        let session_id = created["id"].as_str().unwrap();
