...
use tools::ToolError;
use types::store::MarketplaceStore;
...
impl MarketplaceStore for MarketplaceStoreAdapter {
    ...
    async fn skill_info(&self, id: &str, source: Option<&str>) -> tools::Result<Value> {
        let source_name = Self::provider_name(source);
