...
/// Callback trait for sending replies. Implemented by the host runtime
/// (e.g., ChatDispatcher wraps ChannelRouter + conversation context).
...
impl Tool for ReplyTool {
    ...
    async fn execute(&self, input: Value) -> Result<ToolOutput> {
        let parsed: ReplyInput = serde_json::from_value(input)
            .map_err(|e| crate::error::AiError::Tool(format!("Invalid reply input: {e}")))?;
