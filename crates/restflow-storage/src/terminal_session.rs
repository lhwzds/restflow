...
const TERMINAL_SESSIONS_TABLE: TableDefinition<&str, &[u8]> =
    TableDefinition::new("terminal_sessions");
...
impl TerminalSessionStorage {
    pub fn new(db: Arc<Database>) -> Result<Self> {
        ...
        write_txn.open_table(TERMINAL_SESSIONS_TABLE)?;
        write_txn.commit()?;
    ...
    pub fn put_raw(&self, id: &str, data: &[u8]) -> Result<()> {
        ...
        {
            let mut table = write_txn.open_table(TERMINAL_SESSIONS_TABLE)?;
    ...
    pub fn get_raw(&self, id: &str) -> Result<Option<Vec<u8>>> {
        ...
        let table = read_txn.open_table(TERMINAL_SESSIONS_TABLE)?;
