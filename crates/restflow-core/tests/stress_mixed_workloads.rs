...
use stress_support::{
    ...
    assert_tool_call_result_pairing, chat_smoke_profiles, rounds_for,
    run_chat_workload_with_real_http_io, run_task_workload, task_count_for, task_smoke_profiles,
};

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn smoke_mixed_chat_and_task_workloads_can_run_together() {
    let level = StressLevel::current();
    let chat_profile = chat_smoke_profiles()
        .into_iter()
        .next()
        .expect("chat profile");
    let task_profile = task_smoke_profiles()
        .into_iter()
        .next()
        .expect("task profile");
    let tool_server = MockToolHttpServer::start().await;

    let (chat_summary, task_summary) = tokio::join!(
    ...
    assert_terminal_coverage(&task_summary);
    assert_notifications_within_attempt_budget(&task_summary);
    assert_no_orphan_running(&task_summary);
