...
impl StubHttpLlmFactory {
    ...
    fn resolve_profile(&self, model: &str) -> Option<(ProviderFamily, StreamMode)> {
        ...
        let model_id = restflow_core::ModelId::from_api_name(model)
            .or_else(|| restflow_core::ModelId::from_serialized_str(model))
            .or_else(|| restflow_core::ModelId::from_canonical_id(model))?;
...
fn provider_style_for_model(model: &str) -> ProviderFamily {
    if let Some(model_id) = restflow_core::ModelId::from_api_name(model)
        .or_else(|| restflow_core::ModelId::from_serialized_str(model))
    {
...
impl Tool for StressHttpTool {
    ...
    async fn execute(&self, input: Value) -> restflow_ai::tools::ToolResult<ToolOutput> {
