//! Typed workspace note storage wrapper.

use crate::models::{NoteQuery, NoteStatus, WorkspaceNote, WorkspaceNotePatch, WorkspaceNoteSpec};
...
pub struct WorkspaceNoteStorage {
    inner: restflow_storage::WorkspaceNoteStorage,
}

impl WorkspaceNoteStorage {
    pub fn new(db: Arc<Database>) -> Result<Self> {
        Ok(Self {
            inner: restflow_storage::WorkspaceNoteStorage::new(db)?,
        })
    }

    pub fn create_note(&self, spec: WorkspaceNoteSpec) -> Result<WorkspaceNote> {
    ...
    pub fn get_note(&self, id: &str) -> Result<Option<WorkspaceNote>> {
    ...
    pub fn update_note(&self, id: &str, patch: WorkspaceNotePatch) -> Result<WorkspaceNote> {
        let mut note = self
            .get_note(id)?
    ...
    pub fn list_notes(&self, query: NoteQuery) -> Result<Vec<WorkspaceNote>> {
    ...
    fn read_entries(&self, rows: Vec<(String, Vec<u8>)>) -> Result<Vec<WorkspaceNote>> {
        rows.into_iter()
...
mod tests {
    ...
    fn update_and_query_note() {
        ...
        assert_eq!(updated.status, NoteStatus::InProgress);
