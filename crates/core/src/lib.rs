...
pub mod auth {
    ...
    pub mod manager {
        ...
        use super::types::{
            AuthProfile, AuthProvider, Credential, CredentialSource, ProfileHealth,
        ...
        impl AuthProfileManager {
            ...
            pub async fn get_profiles_for_provider(
                &self,
            ...
            pub async fn get_available_for_provider(
                &self,
                provider: AuthProvider,
            ) -> Vec<AuthProfile> {
                let profiles = self.profiles.read().await;
            ...
            pub async fn get_compatible_profiles_for_model_provider(
                ...
                let mut candidates: Vec<AuthProfile> = profiles
                    .values()
                    .filter(|profile| {
                        profile.is_available() && compatible.contains(&profile.provider)
                    })
                    .cloned()
                    .collect();
            ...
            pub async fn get_available_profile(
                &self,
                provider: AuthProvider,
            ) -> Option<AuthProfile> {
            ...
            pub async fn get_credential_for_model(
                &self,
                provider: Provider,
            ) -> Option<AuthProfile> {
                let compatible = AuthProvider::compatible_with(provider);
            ...
            async fn refresh_expired_profiles(&self, provider: AuthProvider) -> Result<usize> {
                ...
                for (profile_id, secure_credential) in candidates {
                    ...
                    let refresh_token =
                        match self.resolver.resolve_refresh_token(&secure_credential) {
                            Ok(Some(token)) => token,
                            Ok(None) => {
        ...
        mod tests {
            ...
            async fn test_manager_select_profile() {
                ...
                let profile1 =
                    create_test_profile(&secrets, "Low Priority", AuthProvider::Anthropic);
                let mut profile2 =
                    create_test_profile(&secrets, "High Priority", AuthProvider::Anthropic);
            ...
            async fn test_manager_get_compatible_profiles_for_cli_model_provider() {
                ...
                let mut profile =
                    create_test_profile(&secrets, "Claude Code", AuthProvider::ClaudeCode);
                profile.priority = 5;
                let id = manager.add_profile(profile).await.unwrap();
            ...
            async fn test_manager_get_available_profiles() {
                ...
                let mut profile2 =
                    create_test_profile(&secrets, "Disabled", AuthProvider::Anthropic);
    ...
    pub(crate) mod provider_access {
        use crate::provider_policy::{
            profile_provider_resolution_order, provider_access_profiles,
        ...
        mod tests {
            ...
            async fn resolve_model_from_credentials_prefers_dedicated_auth_profiles() {
                ...
                let model = resolve_model_from_credentials(&manager, |key| {
                    secret_exists(secrets.as_ref(), key)
                })
                .await;
    ...
    pub mod types {
        ...
        pub enum AuthProvider {
            ...
            ///
            /// Use explicit profile or secret configuration. RestFlow does not scan local Claude Code files.
            ClaudeCode,
            /// OpenAI API
            #[serde(rename = "openai")]
            OpenAI,
    ...
    pub use provider_access::{provider_available, resolve_model_from_credentials, secret_exists};
...
pub mod provider_policy {
    ...
    pub fn provider_allows_secret_env(provider: Provider) -> bool {
    ...
    mod tests {
        use super::{
            ...
            profile_provider_resolution_order, provider_access_profiles,
            provider_allows_secret_env, provider_auth_providers, provider_default_model,
        ...
        fn agent_has_local_credential(context: &AssessmentContext, agent: &AgentNode) -> bool {
            match agent.api_key_config.as_ref() {
                ...
                Some(ApiKeyConfig::Secret(secret_name)) => {
                    secret_exists(&context.secrets, secret_name)
                }
                None => false,
            }
        }
